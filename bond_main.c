...
const char *bond_mode_name(int mode)
{
	static const char *names[] = {
		...
		[BOND_MODE_NLB] = "new load balancing",
	};

	//if (mode < BOND_MODE_ROUNDROBIN || mode > BOND_MODE_ALB)
	if (mode < BOND_MODE_ROUNDROBIN || mode > BOND_MODE_NLB)
		return "unknown";
...
// jungmin ---------------------------------------------------------------------
    if(bond->params.mode == BOND_MODE_NLB) {
        ret = bond_nlb_recv(skb, bond, slave);
        if(ret == RX_HANDLER_CONSUMED) return ret;
    }
//------------------------------------------------------------------------------
...
// jungmin ---------------------------------------------------------------------
//	INIT_DELAYED_WORK(&bond->nlb_send_work, bond_nlb_send_handler);
//	INIT_DELAYED_WORK(&bond->nlb_recv_work, bond_nlb_recv_handler);
//------------------------------------------------------------------------------
}
...
// jungmin ---------------------------------------------------------------------
//	cancel_delayed_work_sync(&bond->nlb_send_work);
//	cancel_delayed_work_sync(&bond->nlb_recv_work);
...
static int bond_close(struct net_device *bond_dev)
{
	...
	if(BOND_MODE(bond) == BOND_MODE_NLB) bond_nlb_deinitialize(bond);
...
static int __init bonding_init(void)
{
	...
	pr_info("Modified by Jungmin So (Compiled on December 25, 2015)");
